...
export function validateContent(content: GameContent): GameContent {
  ...
  for (const wave of parsed.waves) {
    ...
    if (totalEnemies > MAX_WAVE_ENEMIES) {
      throw new Error(`Wave ${wave.id} exceeds the ${MAX_WAVE_ENEMIES} enemy cap.`);
    }
